 ...
 */
const useMultiSelect = (canvas: fabric.Canvas | null, drawingMode: string) => {
  useEffect(() => {
    if (!canvas) return;
    let selectionRect: fabric.Rect | null = null;
    let origin: { x: number; y: number } | null = null;

    const handleMouseDownMultiSelect = (e: fabric.IEvent) => {
    ...
    const handleMouseMoveMultiSelect = (e: fabric.IEvent) => {
    ...
    const handleMouseUpMultiSelect = (_e: fabric.IEvent) => {
      if (!selectionRect || !origin) return;
